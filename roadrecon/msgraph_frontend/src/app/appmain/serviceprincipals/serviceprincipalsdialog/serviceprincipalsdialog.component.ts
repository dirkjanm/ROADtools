...
import { ServicePrincipalsItem, AppRolesItem, DatabaseService } from '../../msgraphobjects.service'
...
export class ServicePrincipalsdialogComponent {
  ...
  constructor(
    ...
    // TO DO: Metadata property no longer exists - see if there is a way to get this data from a different API call
    ...
    if (sp.appRolesAssigned && sp.appRolesAssigned.length > 0){
      this.service.getAppRolesByResource(sp.id).subscribe((data: AppRolesItem[]) => this.approlesgiven.push(...data));
    }
    if (sp.appRolesAssignedTo && sp.appRolesAssignedTo.length > 0){
      this.service.getAppRolesByPrincipal(sp.id).subscribe((data: AppRolesItem[]) => this.approlesgivento.push(...data));
    }
  }
}
